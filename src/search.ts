...
export async function sogouWeixinSearchAll(
    ...
    for (let page = 1; page <= maxPages; page++) {
        const results = await sogouWeixinSearch(query, page, true, userAgent);
        if (results.length === 0) {
            break;
        }
...
export async function getRealUrlFromSogou(
    sogouUrl: string,
    options: { cookie?: string; referer?: string; userAgent?: string } = {},
): Promise<string> {
    const headers: Record<string, string> = {
        ...
        Connection: 'keep-alive',
        Pragma: 'no-cache',
        'User-Agent': options.userAgent?.trim() ? options.userAgent.trim() : DEFAULT_USER_AGENT,
    };
    if (options.cookie) {
